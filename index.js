const express = require('express');
const admin = require('firebase-admin');
const cors = require('cors');
const app = express();

app.use(express.json());
app.use(cors());

/**
 * FIREBASE ANAHTAR DÜZELTMESİ
 * Buradaki .replace(/\\n/g, '\n') kısmı PEM format hatasını çözen kısımdır.
 */
const serviceAccount = {
  "type": "service_account",
  "project_id": "neuvalcall",
  // Kendi private_key değerini tırnak içine tam olarak yapıştır
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n'),
  "client_email": "firebase-adminsdk-fbsvc@neuvalcall.iam.gserviceaccount.com",
};

// Firebase'i sadece bir kez başlat
if (!admin.apps.length) {
  try {
    admin.initializeApp({
      credential: admin.credential.cert(serviceAccount)
    });
    console.log("Firebase Admin Başarıyla Başlatıldı");
  } catch (error) {
    console.error("Firebase Başlatma Hatası:", error);
  }
}

// 🚀 ANDROID UYUMU: MainActivity.kt içindeki POST isteğini karşılar
app.post('/api/sendCall', async (req, res) => {
  const { fcmToken, callerName, roomId, callerId } = req.body;

  // Güvenlik Kontrolü
  if (!fcmToken || !roomId) {
    console.error("Hata: Eksik Parametre (fcmToken veya roomId yok)");
    return res.status(400).send({ error: "fcmToken ve roomId gereklidir!" });
  }

  // FCM Bildirim Paketi
  const message = {
    token: fcmToken, 
    data: { 
      type: 'hybrid_call', // MyFirebaseMessagingService.kt bu ismi bekliyor
      callerName: String(callerName || "Bilinmeyen Üye"),
      roomId: String(roomId),
      callerId: String(callerId || "")
    },
    android: {
      priority: 'high', // Telefon uykudaysa uyandırmak için şart
      ttl: 0           // Mesajın kuyrukta beklemeden hemen gitmesi için
    }
  };
  
  try {
    const response = await admin.messaging().send(message);
    console.log('Bildirim Gönderildi:', response);
    res.status(200).send({ success: true, messageId: response });
  } catch (error) {
    console.error('FCM Gönderim Hatası (Backend):', error);
    res.status(500).send({ error: error.message });
  }
});

// Ana sayfa testi
app.get('/', (req, res) => {
  res.send('NeuValCall V2.0 Backend Aktif ve Çalışıyor!');
});

// Vercel için Export
module.exports = app;
